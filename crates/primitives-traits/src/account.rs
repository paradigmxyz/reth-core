...
use evm2::{
    bytecode::{Bytecode as Evm2Bytecode, BytecodeDecodeError},
    evm::AccountInfo,
};
...
pub struct Bytecode(pub Evm2Bytecode);
...
impl Bytecode {
    ...
    pub fn new_raw(bytes: Bytes) -> Self {
        Self(Evm2Bytecode::new_raw(bytes))
    }
    ...
    pub fn new_raw_checked(bytecode: Bytes) -> Result<Self, BytecodeDecodeError> {
        Evm2Bytecode::new_raw_checked(bytecode).map(Self)
    }
...
impl From<Evm2Bytecode> for Bytecode {
    #[inline]
    fn from(value: Evm2Bytecode) -> Self {
        Self(value)
    }
}

impl From<Bytecode> for Evm2Bytecode {
    #[inline]
    ...
    fn from_compact(mut buf: &[u8], _: usize) -> (Self, &[u8]) {
        ...
        let decoded = match variant {
            ...
            LEGACY_ANALYZED_BYTECODE_ID => {
                ...
                let jump_table_bytes = jump_table_len.div_ceil(8);
                let jump_table =
                    evm2::bytecode::JumpTable::from_slice(&buf[..jump_table_bytes], jump_table_len);
                buf.advance(jump_table_bytes);
                ...
                Self(unsafe { Evm2Bytecode::new_analyzed(bytes, original_len, jump_table) })
            }
            EIP7702_BYTECODE_ID => {
...
impl From<AccountInfo> for Account {
    fn from(evm_acc: AccountInfo) -> Self {
        Self {
            balance: evm_acc.balance,
            nonce: evm_acc.nonce,
            bytecode_hash: (!evm_acc.code_hash.is_zero() && evm_acc.code_hash != KECCAK_EMPTY)
                .then_some(evm_acc.code_hash),
            #[cfg(feature = "account-ext")]
            extension: AccountExtension::from_shared(evm_acc.extension.into_shared()),
        }
    }
}
    ...
    fn from(evm_acc: &AccountInfo) -> Self {
        Self {
            ...
            bytecode_hash: (!evm_acc.code_hash.is_zero() && evm_acc.code_hash != KECCAK_EMPTY)
                .then_some(evm_acc.code_hash),
            #[cfg(feature = "account-ext")]
            extension: AccountExtension::from_shared(evm_acc.extension.clone().into_shared()),
        }
    }
}
...
mod tests {
    ...
    fn extension_roundtrips_without_copying_shared_payload() {
        ...
        let evm = AccountInfo::from(account.clone());
        assert_eq!(evm.extension.as_ptr(), pointer);
        let restored = Account::from(evm);
